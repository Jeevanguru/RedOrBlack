...
import { AccountsResult, BetsData, PlayerDetails, WebhookData, WebhookKey } from '../interfaces/index.js';
...
export const prepareDataForWebhook = async (betObj: BetsData, key: WebhookKey): Promise<WebhookData | false> => {
    try {
        ...
        if (key == 'DEBIT') return {
            ...
            txn_ref_id: txn_id,
            description: `${winning_amount} credited for Red Or Black Mini game for Round ${id}`,
            txn_type: 1
        }
